...
public class OpenGLEvent implements GLEventListener{
    ...
    public void display(GLAutoDrawable drawable) {
       ...
       if(mew.isDead){
           int x,y,z;
           if(diglett.isFirstPerson && diglett.isFirstPerson){
                x=key.diglett.centroX;
                y=key.diglett.centroY;
                z=key.diglett.centroZ + 5; // set position
           }else{
           }
                x=-10;
                y=-0;
                z=20;
           TextRenderer3D txt3d = new TextRenderer3D(Font.decode(GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames()[0]), 3);
           txt3d.draw("Diglett Wins", x, y, z, 0.2f, key.diglett.isFirstPerson);
           ...
           if(mew.isFirstPerson && key.isFirstPerson){
                x=key.mew.centroX;
                y=key.mew.centroY;
                z=key.mew.centroZ; // set position
           }else{
           }
                x=-10;
                y=-0;
                z=20;
           TextRenderer3D txt3d = new TextRenderer3D(Font.decode(GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames()[0]), 3);
           txt3d.draw("Mew Wins", x, y, z, 0.2f, key.mew.isFirstPerson);
